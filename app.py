...
def new_order():
    ...
    with get_db() as conn:
        ...
        products = conn.execute("SELECT name, part_no, unit_price FROM products ORDER BY name").fetchall()
...
@app.route("/subcontractors/edit/<int:sub_id>", methods=["POST"])
@login_required
...
@app.route("/subcontractors/delete/<int:sub_id>", methods=["POST"])
@login_required
